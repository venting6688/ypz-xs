				...
				elseApi.documentReview(data).then((res) => {
					if (res.data.code === 200) {
						...
						if (data[1]) {
							...
							const str =
								this.list[0] &&
								this.list[0].section.map((item) => {
									let result = '';
									if (item.E07.content !== 'M') {
										result += `${item.E02.content}结果:${item.E03.content == undefined ? item.E05.content : item.E03.content}参考值:${item.E06.content ? item.E06.content : ''}`;
									}
									return result;
								}).filter(Boolean).join();
