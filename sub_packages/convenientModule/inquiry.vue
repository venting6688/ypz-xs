<template>
	<view class="inquiry">
		...
		<!-- <view class="head">请按提示认真填写，以便医生了解您的情况</view> -->
		<view class="center">
							...
							<view class="top2" v-if="x.type == 2">
								...
								<view class="top2-content">
									<view class="department">
										<view class="title">“安好” 推荐您去以下科室挂号</view>
									...
									<view class="doctor">
										<view class="title">“安好” 为您推荐以下医生</view>
										<view class="top2-center">
											<view class="img">
			...
			<!-- 持续周期 -->
			<uni-popup class="Dialog3" ref="popup3" type="bottom" :mask-click="false" :safeArea="false" mask-background-color="transparent">
				<view>
					<picker-view v-if="visible" class="picker-view" :indicator-style="indicatorStyle" :mask-style="maskStyle" :value="multiIndex" @change="bindChange">
						<picker-view-column>
...
export default {
			...
			msgList: [
				{
					my: false,
					type: 1,
					msg: '请描述您的症状，比如：肚子痛、手腕关节疼痛、膝盖肿胀....'
				}
			], //消息集合
	...
	onShow() {
		...
		} else if (options && options.summary) {
			this.scene = false;
			let content = JSON.parse(options.summary);
			this.patient = {
				patientName: this.footData.patientName,
				sex: this.footData.sex,
				age: this.calculateAge(this.footData.idNum),
				deptName: content.doctorName,
				doctorName: content.doctorName
			};
